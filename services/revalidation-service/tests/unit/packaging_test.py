"""Image and compose wiring — Sprint 5 Task 5.4.

Text assertions, deliberately: the failures they catch are the ones a green test
suite hides until a container starts. A pin that drifts from the root
requirements.txt, or recordings that never get copied into the image, is
...
def test_the_driver_is_present() -> None:
    """For `processed_events` dedup only — this service owns no tables of its own."""
...
def test_the_image_carries_both_import_roots() -> None:
    ...
    assert "PYTHONPATH=/app:/app/services/revalidation-service" in DOCKERFILE
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # The live read is still a fixture replay, so the recordings are runtime
        # data for this image, not test data (INPUTS_NEEDED.md item 1).
        "COPY services/revalidation-service/tests/fixtures/",
    ],
)
def test_the_image_copies_what_it_reads(path: str) -> None:
    assert path in DOCKERFILE


def test_the_container_does_not_run_as_root() -> None:
    assert "USER revalidation" in DOCKERFILE


def test_compose_defines_the_service() -> None:
    assert "\n  revalidation-service:" in COMPOSE
    assert "dockerfile: services/revalidation-service/Dockerfile" in COMPOSE
...
def test_compose_waits_for_its_backing_services(dependency: str) -> None:
    """`service_healthy`: the dedup mark is written on the first event read."""
    service = COMPOSE.split("\n  revalidation-service:", 1)[1]
...
def test_compose_points_at_the_container_hostnames() -> None:
    ...
    service = COMPOSE.split("\n  revalidation-service:", 1)[1].split("\nvolumes:", 1)[0]
